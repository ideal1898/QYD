...
using PigRunner.Repository;
using PigRunner.Services;
...
namespace PigRunner.WebApi.Commons.Helpers
{
    ...
    public static class IOCHelper
    {
        ...
        {
            StaticLoadType(services);
            return services;
        }

        private static void StaticLoadType(this IServiceCollection services)
        {
            var ServiceDll = typeof(IBaseService).Assembly;
            var RepositoryDll = typeof(IBaseRepository).Assembly;
            Assembly[] assemblies = new List<Assembly>() { ServiceDll, RepositoryDll }.ToArray();
            if (assemblies.Length > 0)
            {
                StaticRegister(typeof(IScopedService), assemblies, services);
                StaticRegister(typeof(ITransientService), assemblies, services);
